...
OUTPUT_FILE = "dailyViralSongs.json"
...
def login_to_spotify():
    with sync_playwright() as p:
        ...
        # Select the 6th div at the top level (index 5 because it's 0-based)
        daily_top_viral_songs = overview_body.locator(":scope > div").nth(5)


        # Check if the element exists before clicking
        if daily_top_viral_songs.count() > 0:
            print("Daily Top Viral Songs Global found! Clicking...")
            daily_top_viral_songs.click()
            page.wait_for_timeout(3000)  # Wait for the page to load
        else:
            print("Daily Top Viral Songs Global tile not found!")
